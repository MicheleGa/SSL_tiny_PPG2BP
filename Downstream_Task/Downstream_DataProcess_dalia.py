...
def SamplePerClass_Index(config, dataset_array, num_samples):
...
def prepare_dalia_dataset(config, training_persons, test_persons):
    ...
    print(f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Preprocessing {config["dataset_name"]} training set')
    ...
    train_data, label_data = shuffle(train_data, label_data, random_state=config["seed"])
    
    print(f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}: Preprocessing {config["dataset_name"]} test set')
