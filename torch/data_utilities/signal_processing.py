...
from scipy.signal import butter, filtfilt, sosfiltfilt, find_peaks, savgol_filter, welch
...
def hampel_filter_std_dev(signal, window_size=100, n_sigma=3.0):
    """
    Hampel filter with outlier replacement based on standard deviation.

    Args:
        signal: 1D numpy array representing the input signal.
        window_size: Size of the window for outlier detection.
        n_sigma: Number of standard deviations for outlier detection.

    Returns:
        Filtered signal with outliers replaced by the median of the window.
    """
    ...
    rolling_std = np.sqrt(np.convolve(np.square(signal - rolling_mean), np.ones(window_size)/window_size, mode='same'))

    # Calculate upper and lower thresholds
    upper_threshold = rolling_mean + n_sigma * rolling_std
    lower_threshold = rolling_mean - n_sigma * rolling_std

    # Identify and replace outliers
    outlier_indices = np.where((signal > upper_threshold) | (signal < lower_threshold))[0]

    # Calculate rolling median
    rolling_median = np.convolve(signal, np.ones(window_size)/window_size, mode='same') 

    filtered_signal[outlier_indices] = rolling_median[outlier_indices]
...
def load_and_preprocess_mimic_iii_data(config, idx, id_data, output_file):
    ...
    b,a = butter(filter_order,[0.5, 8], 'bandpass', fs=fs)
    ...
    for record in os.listdir(id_data):
        ...
        for i in range(0, n_win):
            ...
            # Normalize PPG
            min_val = np.min(ppg_win)
            max_val = np.max(ppg_win)
            ppg_win = (ppg_win - min_val) / (max_val - min_val)
